...
<style>
    ...
    .delIcon:hover, .checkedbox:hover, .allCheck:hover, .proDetail:hover{cursor:pointer;}
</style>
</head>
<body>
        ...
        <div class="outer">
            <div class="outer-1">
                    ...
                    <tbody>
                        ...
                        <% }else{ %>
                            ...
                            <tr>
                              	...
                              	<td style="vertical-align:middle" width="12%" align="center"><img class="proDetail" onclick="location.href='<%= contextPath %>/proDetail.pro?pno=<%= p.getProductNo() %>'" src="<%= p.getProductImg() %>" width="100px;"></td>
                              	<td style="vertical-align:middle" width="45%" class="proDetail" onclick="location.href='<%= contextPath %>/proDetail.pro?pno=<%= p.getProductNo() %>'"><%= p.getProductName() %></td>
                              	<td style="vertical-align:middle" align="center">
                                  	...
                                  	<input type="hidden" class="proNumber" name="proNo" value="<%= p.getProductNo() %>">
                                  	<input type="hidden" class="proStock" name="proStock" value="<%= p.getStock() %>">
                ...
                <div>
					...
					<% if(!list.isEmpty()){ %>
                     	<button type="button" onclick="checkedOrder();" class="btn btn-primary">선택 상품 주문</button>
                     	<button type="button" onclick="allOrder();" class="btn btn-primary">전체 상품 주문</button>
                    <% } %>
                </div>
            </div>
        </div>
    ...
    <script>
    	...
    	function checkedOrder(){
    		if($(".checkedbox:checked").parent().parent().find(".proNumber").val() == null){
    			alert("선택된 상품이 없습니다.");
    		}else{
    			let soldOutNo = new Array();
    			$(".checkedbox:checked").each(function(){
        			if($(this).parent().parent().find(".proStock").val() == 0){
        				soldOutNo.push($(this).parent().parent().find(".proNumber").val());
        			}
        		});
    			if(soldOutNo[0] != null){
    				alert("품절된 상품이 포함되어있습니다.")
    			}else{
	    			let proNo = new Array();
	        		$(".checkedbox:checked").each(function(){
    	...
    	function allOrder(){
    		let soldOutNo = new Array();
    		$(".proStock").each(function(){
    			if($(this).val() == 0){
    				soldOutNo.push($(this).prev().val());
    			}
    		});
    		if(soldOutNo[0] != null){
				alert("품절된 상품이 포함되어있습니다.")
		...
		$(function(){
			...
			$(".proStock").each(function(){
    			if($(this).val() == 0){
    				...
    				$(this).parent().prev().html("<s style='color:red'>" + name + "</s> <b>품절</b>");
    			}
			});
    	});
