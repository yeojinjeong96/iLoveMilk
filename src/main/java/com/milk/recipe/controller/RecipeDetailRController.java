 ...
 */
@WebServlet("/detailR.re")
public class RecipeDetailRController extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public RecipeDetailRController() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(result > 0) {
			...
			request.getRequestDispatcher("views/recipe/recommendRecipeDetailView.jsp").forward(request, response);
			
		
		}else {
			
			request.setAttribute("errorMsg", "레시피 상세조회에 실패했습니다.");
			request.getRequestDispatcher("views/recipe/recommendRecipeListView.jsp").forward(request, response);
		}
